...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ndump",
        default=0,
        help="specifies the number of iterations between any diagnostic or restart file dumps. Default is 0",
        type=int)
    ...
    simulation.add_argument(
        ...
        "--algorithm",
        type=str,
        help="specifies the algorithm to be used in the simulation. Default is 'standard'",
        default="standard",
        choices=[
            "standard",
            "quasi_3d",
    ...
    time.add_argument(
        ...
        default=(
            ...
            0),
        type=tuple,
        help="specify the initial and final time of the simulation in [s]. Default is (0,00).")
    ...
    el_mag_fld.add_argument(
        ...
        default="none",
        type=str,
        help="controls the type of smoothing to be applied to the EM fields. Default is 'none'",
        choices=[
            "none",
            "stand",
            "local"])
    ...
    plasma.add_argument(
        ...
        default=0,
        type=int,
        help="specifies the particle buffer size for each node. Default is 0")
    ...
    beam.add_argument(
        ...
        default=100e-15,
        type=float,
        help="the overall beam charge in [C]. Default is 100fC.")
    ...
    with open(args.filename, "w+") as file:
        ...
        file.write(
            f"if_periodic(1:{args.dimension}) = " + str(
                args.if_periodic).replace(
                "(", ".").replace(
                ")", ".").replace(
                    ",", ".,.").replace(
                        "'", "").replace(
        ...
        boundaries = [list(
            map(lambda x: x / skin_depth(args.plasma_density), i))for i in window_lengths]

        window_lengths = [abs(window_lengths[1][i] - window_lengths[0][i])
        ...
        file.write(f"xmin(1:{args.dimension}) = " +
                   str(boundaries[0]).replace("[", "").replace("]", ",\n"))

        file.write(f"xmax(1:{args.dimension}) = " +
                   str(boundaries[1]).replace("[", "").replace("]", ",\n"))
        ...
        file.write(
            f"tmin = {args.time[0] / skin_depth(args.plasma_density)},\n")
        file.write(
            f"tmax = {args.time[1]/ skin_depth(args.plasma_density)},\n")
